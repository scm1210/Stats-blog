...
```{r}
# Calculate the mean and standard deviation of wwwhr, and identify outliers
data_pos_filtered <- data_pos %>%
  ...
  mutate(is_outlier = if_else(wwwhr < (mean_wwwhr - 3 * sd_wwwhr) | wwwhr > (mean_wwwhr + 3 * sd_wwwhr), TRUE, FALSE)) %>%
  
  # Count the number of outliers
  summarise(outliers_count = sum(is_outlier, na.rm = TRUE)) %>%
  pull(outliers_count) -> outliers_count
...
# Print the number of outliers and check the filtered data
...
```{r}
num_zeros <- sum(data_pos_filtered$wwwhr == 0)

# Predict the expected number of zeros using both models
predicted_zeros <- c(
  Poisson = sum(predict(poisson_model_filtered, type = "response") == 0),
  NegativeBinomial = sum(predict(nb_model, type = "response") == 0)
)

# Compare observed and predicted zeros for both models
results <- ifelse(num_zeros > predicted_zeros, 
                  paste("Zero Inflation detected in", names(predicted_zeros), "model"), 
                  paste("No Zero Inflation in", names(predicted_zeros), "model"))

# Output the results
cat("Observed Zeros: ", num_zeros, "\n")
cat("Predicted Zeros (Poisson Model): ", predicted_zeros["Poisson"], "\n")
cat("Predicted Zeros (Negative Binomial Model): ", predicted_zeros["NegativeBinomial"], "\n")
cat(results, "\n")
```
