...
library(pacman)
pacman::p_load(xaringanExtra,tidyverse,lme4,patchwork,ggdist,install = T)
...
knitr::include_graphics("~/Library/CloudStorage/GoogleDrive-sm9518@princeton.edu/My Drive/Classes/Stats-blog/posts/Lab-6/images/toys.png")
```
:::
...
ggplot(toy2, aes(x = hrs_week)) + 
  ...
  facet_wrap(~toy_type) +
  geom_line(aes(y=pred_lm), col = "red") + plot_aes
```
...
ggplot(toy2, aes(x = hrs_week)) + 
  geom_point(aes(y = R_AGE), size=1, alpha=.3) +
  facet_wrap(~toy_type) +
  geom_line(aes(y=pred_lm), col = "red",linetype = 2) + plot_aes
...
ggplot(toy2, aes(x = hrs_week)) + 
  geom_point(aes(y = R_AGE), size=1, alpha=.3) +
  facet_wrap(~toy_type) +
  geom_line(aes(y=pred_fe), col = "blue") + plot_aes
```
...
ggplot(toy2, aes(x = hrs_week)) + 
  geom_point(aes(y = R_AGE), size=1, alpha=.3) +
  facet_wrap(~toy_type) +
  geom_line(aes(y=pred_ri), col = "green") + plot_aes
```
...
ggplot(toy2, aes(x = hrs_week)) + 
  geom_point(aes(y = R_AGE), size=1, alpha=.3) +
  facet_wrap(~toy_type) +
  geom_line(aes(y=pred_rs), col = "orange") + plot_aes
```

`r qend()` We should be using the model with the random slopes and intercepts as the data is nested and each toy ahs the different slopes and intercepts.
...
# I figured we would need to specify random slops and intercepts as the data is nested and each toy has different slopes and intercepts.
...
-   where the black line cuts the y axis (at x=0) `the grand mean or intecept`
-   the slope of the black line `the fixed effect`
-   the standard deviation of the distances from all the individual datapoints (toys) to their respective toy-type lines `the residual standard deviation`
-   the standard deviation of the distances from all the toy-type lines to the black line  `the random effect standard deviation`
:::
...
effect(term = "audio*headphones", mod = sdmt_mod) |>
  ...
  ggplot(aes(x=audio,y=fit,
             ...
             col=headphones))+
  geom_pointrange(size=1,lwd=1) + plot_aes
```
