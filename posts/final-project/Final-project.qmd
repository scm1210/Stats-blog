...
pacman::p_load(tidyverse, DT, broom, performance,
               ...
               emmeans,knirt,MASS,brant,devtool,purr,
               ggridges,specr, furrr,install = TRUE)
...
Now that we have a sense of the various relationships, we can fit various models to predict our outcome (win/loss).

We will do so using a **specification curve analysis (SCA)** framework, via the `Specr` package in R. You can learn more about Specr [here](https://cran.r-project.org/web/packages/specr/vignettes/specr.html).
...
# write function to extract r squared
tidy_new <- function(x) {
  fit <- broom::tidy(x, conf.int = TRUE)
  
  # Try to extract R2 info safely
  r2_vals <- tryCatch(
    performance::r2(x),
...
```{r}
#| label: formulas
### filter out the models with no covariates
...
Now that we have set up the specifications and defined the formulas, we can run the models. The `specr` package allows us to run the models in parallel and extract the results in a tidy format, we'll utilize `furrr` to run our jobs in parallel.
...
if (!file.exists(model_path)) {
  ...
  opts <- furrr_options(
    globals = list(glmer_binomial = glmer_binomial)
  )
  ...
  plan(sequential) # switch back to sequential
...
Now taht we used `specr()`, we can summarize individual specifications by using `broom::tidy()` and `broom::glance()`. For most cases, this provides a sufficient and appropriate summary of the relationship of interest and model characteristics. Sometimes, however, it might be useful to investigate specific models in more detail or to investigate a specific parameter that is not provided by the two functions (e.g., r-square).

### Inspect the significant models

First, we'll look at just the significant models. This is done by filtering the results dataframe to only include models with a p-value less than 0.05. We can then use the `DT` package to create an interactive table that allows us to sort and filter the data.
...
Now that we have a sense of the best model, we can plot the results using `ggplot` to create a bargraph that shows us how much variance each of the top 50 models accounts for, as well as which might be the best one.
